...
const EditBowler = ({ isOpen, onClose, onUpdate, bowler, session }) => {
  ...
  // Update form data when bowler prop changes
  useEffect(() => {
    if (bowler) {
      setFormData({
        name: bowler.name || '',
        average: bowler.average?.toString() || '',
        lane: bowler.lane?.toString() || '',
        scratch_brackets: bowler.scratch_brackets?.toString() || '0',
        handicap_brackets: bowler.handicap_brackets?.toString() || '0',
        high_game_scratch: bowler.high_game_scratch || false,
        high_game_handicap: bowler.high_game_handicap || false,
        eliminator: bowler.eliminator || false
      })
      setErrors({})
    }
  }, [bowler])
  ...
  const handleSubmit = useCallback(async (e) => {
    ...
    try {
      ...
      await onUpdate(bowler.id, bowlerData)
      onClose()
    } catch (error) {
  ...
  }, [formData, validateForm, onUpdate, bowler, session, onClose])
  ...
  return (
    <Modal
      isOpen={isOpen}
      onClose={handleClose}
      title={`Edit Bowler - ${session.name}`}
      className="max-w-2xl"
    >
